...
#   4. For each gatingIssue, when network access to `gh` is available
#      the issue is fetched; if it is `CLOSED` while the marker's
#      `reason` is still `awaiting_<n>`, FAIL. `--allow-offline` skips
#      this check with a deterministic SKIP marker so air-gapped CI
#      stays green.
#
...
def check_gating_issue_state(num: int, repo: str) -> tuple[str, str]:
    ...
    if proc.returncode != 0:
        # gh prints an error (auth, network, deprecation warning that
        # still exits non-zero, etc.). Treat as unknown rather than FAIL
        # so offline / unauthenticated runs do not flake the bundle.
...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-offline",
        action="store_true",
        help="Skip the gh-backed gating-issue state check (emit SKIP marker).",
    )
    parser.add_argument(
        "--repo",
    ...
    # 4. gating issue state.
    if args.allow_offline:
        for entry in markers:
            emit_out(
                f"M7_MARKER:SKIP:{entry.get('name')}:gating_issue_check_offline"
            )
    else:
        for entry in markers:
            name = entry["name"]
            ...
            if not isinstance(gating, int):
                ...
                failures += 1
                continue
            state, detail = check_gating_issue_state(gating, args.repo)
            if state == "UNKNOWN":
                emit_out(f"M7_MARKER:SKIP:{name}:gating_issue_unknown:{detail}")
                continue
            if state == "OPEN":
                emit_out(f"M7_MARKER:PASS:{name}:gating_issue_open:{gating}")
                continue
            # CLOSED
            m = GH_ISSUE_RE.match(reason)
            if m and int(m.group(1)) == gating:
                emit_err(
                    f"M7_MARKER:FAIL:{name}:gating_issue_closed_but_reason_still_awaiting:{gating}"
                )
                failures += 1
            else:
                ...
                emit_out(
                    f"M7_MARKER:PASS:{name}:gating_issue_closed_reason_retargeted:{gating}"
                )
