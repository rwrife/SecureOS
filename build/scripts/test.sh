...
case "$TEST_NAME" in
  ...
  sosh_contract_registry_drift)
    ...
    ;;
  validate_m7_markers)
    # Issue #494: cross-check that tests/m7_toolchain/markers.json stays
    # consistent with the `toolchain_*` case arms in this script, the
    # TEST_TARGETS array in validate_bundle.sh, the per-marker stubs in
    # tests/m7_toolchain/, and the OPEN state of each `gatingIssue`.
    # Same drift-gate shape as #234 (registry), #297 (abi stamps), and
    # #351 (sosh contract).
    run_script "$ROOT_DIR/build/scripts/validate_m7_markers.sh"
    ;;
  m7_markers_drift)
    # Issue #494: negative-canary self-test — mutates a sandbox copy of
    # markers.json to rename a marker (without paired test.sh /
    # TEST_TARGETS / stub renames) and asserts the validator emits the
    # M7_MARKER:FAIL:missing_test_sh_arm / orphan_test_sh_arm /
    # missing_test_targets / orphan_test_targets / script_missing
    # markers. Mirrors the canary discipline from #213 / #234 / #297 /
    # #351.
