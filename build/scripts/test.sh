...
case "$TEST_NAME" in
  ...
  launcher_owner_kind_audit_marker)
    # Issue #554: host gate that pins launch.granted / launch.denied
    # owner_kind=<internal|external|local> contract literals against the
    # owner fixture manifests plus default-when-omitted behavior.
    # Runtime QEMU emit assertions remain SKIP-pinned under #410.
    run_script "$ROOT_DIR/build/scripts/test_launcher_owner_kind_audit_marker.sh"
    ;;
  console_unsigned_bin_auth_marker)
    # Issue #542: host-source pin for `console_authorize_unsigned_binary`
    # marker literals (`[codesign] ... decision=allow|deny[ (cached)]`) and
    # `AUTH_TYPE_UNSIGNED_BIN == 1` in user/include/secureos_api.h.
    # Prevents unsigned-run audit marker drift ahead of #410 runtime wiring.
    run_script "$ROOT_DIR/build/scripts/test_console_unsigned_bin_auth_marker.sh"
    ;;
  process_find_aspace_by_subject)
  ...
  clib_posix_fd)
    ...
    # over `os_fs_read_file` with deterministic host stubs. Pins
    # open/close/read/lseek/unlink symbol presence and the current
    # read-only snapshot semantics.
    run_script "$ROOT_DIR/build/scripts/test_clib_posix_fd.sh"
    ;;
  clib_stdio_fdopen)
    # Issue #766 (DEMO-02 link slice): stdio<->posix_fd fdopen adoption
    # host gate. Links the real shipping shape (stdio.c + posix_fd.c +
    # errno.c) and pins TinyCC's tccelf.c open->fdopen->fwrite->fclose
    # persistence + descriptor-ownership contract.
    run_script "$ROOT_DIR/build/scripts/test_clib_stdio_fdopen.sh"
    ;;
  clib_tinycc_link_surface)
    # M7-TOOLCHAIN-005 follow-up (issue #539): host link/runtime gate for
    # the remaining TinyCC hosted-libc compatibility surface
    # (realloc/free/sprintf/exit/time/localtime/getcwd/getenv/realpath/dl*).
    run_script "$ROOT_DIR/build/scripts/test_clib_tinycc_link_surface.sh"
    ;;
  clib_stddef)
  ...
  apps_dev_include_set)
    ...
    # Enforces strict set parity for the full namespaced include surface;
    # only secureos_api.h remains pending behind #531.
  ...
  kernel_filedemo)
    ...
    ;;
  kernel_binfs)
    # Issue #765: guest-evidence gate for the binary-safe bytes I/O path.
    ...
    if [ -n "$_qemu_to_saved" ]; then
      export SECUREOS_QEMU_TIMEOUT_SECONDS="$_qemu_to_saved"
    else
      unset SECUREOS_QEMU_TIMEOUT_SECONDS
    fi
    ;;
  kernel_edit)
    # Issue #769: guest-evidence gate for the /apps/edit line editor.
    # Boots the ordinary ISO + seeded disk, runs the staged signed
    # /apps/edit.bin app through the console with disk-IO consent, and
    # asserts the EDIT:* markers plus a final cat proving the persisted
    # bytes match the last saved version (denied saves leave storage
    # untouched). Six consent decisions stretch the boot+script window
    # past the 10s default, so raise it (exported so the bash child sees
    # it; restored afterwards to keep other targets on the default).
    _qemu_to_saved="${SECUREOS_QEMU_TIMEOUT_SECONDS:-}"
    export SECUREOS_QEMU_TIMEOUT_SECONDS="${SECUREOS_QEMU_TIMEOUT_SECONDS:-90}"
    run_script "$ROOT_DIR/build/scripts/build_kernel_image.sh"
    run_script "$ROOT_DIR/build/scripts/build_disk_image.sh"
    run_script "$ROOT_DIR/build/scripts/run_qemu.sh" --test kernel_edit
    if [ -n "$_qemu_to_saved" ]; then
      export SECUREOS_QEMU_TIMEOUT_SECONDS="$_qemu_to_saved"
    else
      unset SECUREOS_QEMU_TIMEOUT_SECONDS
  ...
  process_exit_wrapper)
    ...
    ;;
  process_exit_qemu)
    # Issue #551: bridge-level starter gate for `os_process_exit`
    # round-trip status capture and `os_process_spawn`
    # out_exit_status propagation. This is an incremental host
    # preflight while the full launcher/QEMU end-to-end harness
    # is in progress.
    run_script "$ROOT_DIR/build/scripts/test_process_exit_qemu.sh"
    ;;
  ...
  native_fs_bytes_wrapper)
    # Issue #765 (DEMO-01): host-side dynamic round-trip for the
    # binary-safe `os_fs_read_file_bytes` / `os_fs_write_file_bytes`
    # wrappers. Drives a synthetic v5 bridge page: exact-byte payloads
    # with leading/interior/trailing NULs, the pre-v5 version-handshake
    # degrade, every bridge return-code mapping, and the argument
    # guards. Same shape as the process_exit/process_spawn `_qemu`
    # bridge-page gates.
    run_script "$ROOT_DIR/build/scripts/test_native_fs_bytes_wrapper.sh"
    ;;
  clib_os_brk)
  ...
  mem_brk_arena_cap_deny)
    # Issue #558: host-side deny-marker gate for mem_brk arena clamp.
    # Drives `app_native_mem_brk` directly and asserts over-cap growth
    # returns deny + emits canonical CAP:DENY mem_brk marker evidence,
    # while shrink-underflow deny remains marker-silent.
    run_script "$ROOT_DIR/build/scripts/test_mem_brk_arena_cap_deny.sh"
    ;;
  sdk_abi_pin)
