#!/usr/bin/env bash
# build_kernel_entry.sh - Compile kernel assembly and C source into kernel.elf
#
# This script runs INSIDE the Docker toolchain container. It compiles all
# kernel components (entry.asm + C files) and links them into kernel.elf.
# Called by: scripts/build.sh (via docker run)
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
OUT_DIR="$ROOT_DIR/artifacts/kernel"

mkdir -p "$OUT_DIR"

# Compile entry.asm as elf64 (contains both 32-bit and 64-bit code for the
# Multiboot -> long-mode transition)
nasm -f elf64 kernel/arch/x86/boot/entry.asm -o artifacts/kernel/entry.o
nasm -f elf64 kernel/arch/x86/idt_stubs.asm -o artifacts/kernel/idt_stubs.o

# All C kernel code compiled for x86-64 freestanding
CFLAGS="--target=x86_64-unknown-none-elf -ffreestanding -fno-stack-protector -mno-red-zone -mno-mmx -mno-sse -mno-sse2"
clang $CFLAGS -c kernel/arch/x86/idt.c -o artifacts/kernel/idt.o

# Assembly files (cooperative context switch)
clang $CFLAGS -c kernel/core/ctx_switch.S -o artifacts/kernel/ctx_switch.o

clang $CFLAGS -c kernel/core/kmain.c -o artifacts/kernel/kmain.o
clang $CFLAGS -c kernel/core/console.c -o artifacts/kernel/console.o
clang $CFLAGS -c kernel/core/session_manager.c -o artifacts/kernel/session_manager.o
clang $CFLAGS -c kernel/core/boot_banner.c -o artifacts/kernel/boot_banner.o
clang $CFLAGS -c kernel/sched/scheduler.c -o artifacts/kernel/scheduler.o
clang $CFLAGS -c kernel/drivers/disk/ata_pio.c -o artifacts/kernel/ata_pio.o
clang $CFLAGS -c kernel/arch/x86/debug_exit.c -o artifacts/kernel/debug_exit.o
clang $CFLAGS -c kernel/arch/x86/serial.c -o artifacts/kernel/serial.o
clang $CFLAGS -c kernel/arch/x86/vga.c -o artifacts/kernel/vga.o
clang $CFLAGS -c kernel/cap/cap_table.c -o artifacts/kernel/cap_table.o
clang $CFLAGS -c kernel/lib/kmem.c -o artifacts/kernel/kmem.o
clang $CFLAGS -c kernel/cap/capability.c -o artifacts/kernel/capability.o
clang $CFLAGS -c kernel/cap/cap_deny_marker.c -o artifacts/kernel/cap_deny_marker.o
clang $CFLAGS -c kernel/cap/cap_gate.c -o artifacts/kernel/cap_gate.o
clang $CFLAGS -c kernel/event/event_bus.c -o artifacts/kernel/event_bus.o
clang $CFLAGS -c kernel/hal/network_hal.c -o artifacts/kernel/network_hal.o
clang $CFLAGS -c kernel/hal/serial_hal.c -o artifacts/kernel/serial_hal.o
clang $CFLAGS -c kernel/hal/input_hal.c -o artifacts/kernel/input_hal.o
clang $CFLAGS -c kernel/hal/mouse_hal.c -o artifacts/kernel/mouse_hal.o
clang $CFLAGS -c kernel/hal/storage_hal.c -o artifacts/kernel/storage_hal.o
clang $CFLAGS -c kernel/hal/video_hal.c -o artifacts/kernel/video_hal.o
clang $CFLAGS -c kernel/hal/hal_cap_entry.c -o artifacts/kernel/hal_cap_entry.o
clang $CFLAGS -c kernel/drivers/disk/ramdisk.c -o artifacts/kernel/ramdisk.o
clang $CFLAGS -c kernel/drivers/network/virtio_net.c -o artifacts/kernel/virtio_net.o
clang $CFLAGS -c kernel/drivers/serial/pc_com.c -o artifacts/kernel/pc_com.o
clang $CFLAGS -c kernel/drivers/input/ps2_keyboard.c -o artifacts/kernel/ps2_keyboard.o
clang $CFLAGS -c kernel/drivers/input/ps2_mouse.c -o artifacts/kernel/ps2_mouse.o
clang $CFLAGS -c kernel/drivers/video/vga_text.c -o artifacts/kernel/vga_text.o
clang $CFLAGS -c kernel/drivers/video/vga_gfx.c -o artifacts/kernel/vga_gfx.o
clang $CFLAGS -c kernel/drivers/video/framebuffer_text_stub.c -o artifacts/kernel/framebuffer_text_stub.o
clang $CFLAGS -c kernel/drivers/video/gpio_text_stub.c -o artifacts/kernel/gpio_text_stub.o
clang $CFLAGS -c kernel/crypto/sha512.c -o artifacts/kernel/sha512.o
clang $CFLAGS -c kernel/crypto/ed25519.c -o artifacts/kernel/ed25519.o
clang $CFLAGS -c kernel/crypto/cert.c -o artifacts/kernel/cert.o
clang $CFLAGS -c kernel/format/sof.c -o artifacts/kernel/sof.o
clang $CFLAGS -c kernel/fs/fs_service.c -o artifacts/kernel/fs_service.o
clang $CFLAGS -c kernel/user/native_net_service.c -o artifacts/kernel/native_net_service.o
clang $CFLAGS -c kernel/user/app_native_heap.c -o artifacts/kernel/app_native_heap.o
clang $CFLAGS -c kernel/user/app_native_spawn.c -o artifacts/kernel/app_native_spawn.o
clang $CFLAGS -c kernel/user/launcher_exec.c -o artifacts/kernel/launcher_exec.o
clang $CFLAGS -c kernel/ipc/ipc_port.c -o artifacts/kernel/ipc_port.o
clang $CFLAGS -c kernel/svc/console_svc.c -o artifacts/kernel/console_svc.o
clang $CFLAGS -c kernel/svc/fs_svc.c -o artifacts/kernel/fs_svc.o
clang $CFLAGS -c kernel/svc/broker_svc.c -o artifacts/kernel/broker_svc.o
clang $CFLAGS -c kernel/drivers/clock/cmos_rtc.c -o artifacts/kernel/cmos_rtc.o
clang $CFLAGS -c kernel/clock/clock_service.c -o artifacts/kernel/clock_service.o
clang $CFLAGS -c kernel/hal/clock_hal.c -o artifacts/kernel/clock_hal.o
clang $CFLAGS -c kernel/mem/kheap.c -o artifacts/kernel/kheap.o
clang $CFLAGS -c kernel/gfx/vfb_font.c -o artifacts/kernel/vfb_font.o

ld.lld -m elf_x86_64 -T kernel/arch/x86/boot/linker.ld \
  -Map=artifacts/kernel/kernel.map \
  -o artifacts/kernel/kernel.elf \
  artifacts/kernel/entry.o artifacts/kernel/idt_stubs.o artifacts/kernel/idt.o artifacts/kernel/ctx_switch.o artifacts/kernel/kmain.o artifacts/kernel/console.o artifacts/kernel/session_manager.o artifacts/kernel/boot_banner.o artifacts/kernel/scheduler.o artifacts/kernel/ata_pio.o artifacts/kernel/debug_exit.o artifacts/kernel/serial.o artifacts/kernel/vga.o artifacts/kernel/cap_table.o artifacts/kernel/kmem.o artifacts/kernel/capability.o artifacts/kernel/cap_deny_marker.o artifacts/kernel/cap_gate.o artifacts/kernel/event_bus.o artifacts/kernel/network_hal.o artifacts/kernel/serial_hal.o artifacts/kernel/input_hal.o artifacts/kernel/mouse_hal.o artifacts/kernel/storage_hal.o artifacts/kernel/video_hal.o artifacts/kernel/hal_cap_entry.o artifacts/kernel/ramdisk.o artifacts/kernel/virtio_net.o artifacts/kernel/pc_com.o artifacts/kernel/ps2_keyboard.o artifacts/kernel/ps2_mouse.o artifacts/kernel/vga_text.o artifacts/kernel/vga_gfx.o artifacts/kernel/framebuffer_text_stub.o artifacts/kernel/gpio_text_stub.o artifacts/kernel/sha512.o artifacts/kernel/ed25519.o artifacts/kernel/cert.o artifacts/kernel/sof.o artifacts/kernel/fs_service.o artifacts/kernel/native_net_service.o artifacts/kernel/launcher_exec.o artifacts/kernel/app_native_heap.o artifacts/kernel/app_native_spawn.o artifacts/kernel/ipc_port.o artifacts/kernel/console_svc.o artifacts/kernel/fs_svc.o artifacts/kernel/broker_svc.o artifacts/kernel/cmos_rtc.o artifacts/kernel/clock_service.o artifacts/kernel/clock_hal.o artifacts/kernel/kheap.o artifacts/kernel/vfb_font.o

if command -v llvm-objdump >/dev/null 2>&1; then
  llvm-objdump -h artifacts/kernel/kernel.elf > artifacts/kernel/kernel.sections.txt
else
  objdump -h artifacts/kernel/kernel.elf > artifacts/kernel/kernel.sections.txt
fi

echo "Built artifacts/kernel/kernel.elf"