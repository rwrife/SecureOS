...
#
# Thin wrapper around tools/validate_manifests.py so manifest schema
# validation follows the same build/scripts/* entrypoint convention as
# other validators. Mirror script: build/scripts/validate_manifests.sh.
#
# Exit codes:
...
if (-not (Get-Command $Py -ErrorAction SilentlyContinue)) {
    # Fallback to Windows launcher name.
    $Py = 'python'
    if (-not (Get-Command $Py -ErrorAction SilentlyContinue)) {
