/**
 * @file udp.c
 * @brief UDP datagram send/receive for SecureOS.
 *
 * Purpose:
 ...
 *
 * Interactions:
 *   - ipv4.c: udp_send() wraps ipv4_send(); udp_dispatch() is called back
 *     from ipv4_poll().
 *   - dns.c: calls udp_send() for DNS queries and udp_recv() for replies.
 *
 * Launched by:
...
/* Single-slot receive buffer used by dns.c */
...
/* -----------------------------------------------------------------------
 * UDP pseudo-header checksum (optional for IPv4; we send 0 = disabled)
 * --------------------------------------------------------------------- */
...
int udp_send(uint32_t dst_ip, uint16_t src_port, uint16_t dst_port,
  ...
  udp_buf[6] = 0u; /* checksum high (disabled) */
  udp_buf[7] = 0u; /* checksum low  (disabled) */
