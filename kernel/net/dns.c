/**
 * @file dns.c
 * @brief DNS A-record resolver for SecureOS.
 *
 * Purpose:
 *   Sends a minimal DNS A-record query (RFC 1035) over UDP port 53 to
 *   the QEMU user-net DNS resolver (10.0.2.3) and parses the first A-record
 *   in the response.  If the input string is already a dotted-decimal IPv4
 *   literal it is parsed directly without a network round-trip.
 *
 * Interactions:
 *   - udp.c: DNS packets are sent/received as UDP datagrams.
 *   - http.c: calls dns_resolve() for hostname-based URLs.
 *
 * Launched by:
 *   Called on-demand from http.c.  Not a standalone process; compiled
...
/* DNS local source port (fixed; no port randomisation in v1) */
#define DNS_LOCAL_PORT 12053u

enum {
  DNS_FALLBACK_PUBLIC = 0x08080808u, /* 8.8.8.8 */
};

/* -----------------------------------------------------------------------
 * Helpers: string operations (no libc)
 * --------------------------------------------------------------------- */
...
static uint32_t dns_get_be32(const uint8_t *buf, size_t off) {
  return ((uint32_t)buf[off] << 24u) |
         ((uint32_t)buf[off + 1u] << 16u) |
         ((uint32_t)buf[off + 2u] << 8u) |
         (uint32_t)buf[off + 3u];
}

/* Skip a DNS name at *io_pos. Supports RFC 1035 compression pointers. */
...
/* -----------------------------------------------------------------------
 * Public: parse dotted-decimal IPv4 literal
 * Returns IP in host byte order, or 0 if not a valid literal.
 * --------------------------------------------------------------------- */
...
/* -----------------------------------------------------------------------
 * DNS query builder
 * Simple A-record query for hostname.  Returns packet length in buf_out.
 * --------------------------------------------------------------------- */
...
static size_t dns_build_query(const char *hostname, uint8_t *buf_out,
  ...
  buf_out[4] = 0x00u;
  buf_out[5] = 0x01u;
  buf_out[6] = 0x00u; buf_out[7] = 0x00u; /* ANCOUNT */
  buf_out[8] = 0x00u; buf_out[9] = 0x00u; /* NSCOUNT */
  buf_out[10] = 0x00u; buf_out[11] = 0x00u; /* ARCOUNT */
  pos = 12u;

  /* Encode QNAME (labels, each prefixed by length) */
  while (*cursor != '\0' && pos + 2u < buf_size) {
    label_start = pos;
    ++pos; /* reserve space for label length byte */
    label_len = 0u;
  ...
  buf_out[pos++] = 0x00u;  /* root label */
  buf_out[pos++] = 0x00u;  buf_out[pos++] = 0x01u; /* QTYPE=A */
  buf_out[pos++] = 0x00u;  buf_out[pos++] = 0x01u; /* QCLASS=IN */
...
/* -----------------------------------------------------------------------
 * DNS response parser – extract first A-record answer
 * Returns IP in host byte order; 0 on error.
 * --------------------------------------------------------------------- */
...
static uint32_t dns_parse_response(const uint8_t *buf, size_t len, uint16_t txid) {
  ...
  /* Must be response and not an RCODE failure. */
  if ((flags & 0x8000u) == 0u) {
    return 0u;
  }
  if ((flags & 0x000Fu) != 0u) {
  ...
  for (i = 0u; i < ancount; ++i) {
    uint16_t rtype = 0u, rclass = 0u, rdlength = 0u;
...
/* -----------------------------------------------------------------------
 * Public: resolve hostname to IPv4 (host byte order)
 * --------------------------------------------------------------------- */
...
uint32_t dns_resolve(const char *hostname) {
  ...
  for (resolver_idx = 0u; resolver_idx < 2u && resolved == 0u; ++resolver_idx) {
    for (attempt = 0; attempt < DNS_TIMEOUT_ITERATIONS; ++attempt) {
      udp_send(resolvers[resolver_idx], DNS_LOCAL_PORT, DNS_PORT, query_buf, query_len);
      reply_len = udp_recv(DNS_LOCAL_PORT, reply_buf, sizeof(reply_buf), UDP_RECV_TIMEOUT * 2u);
