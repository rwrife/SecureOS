/**
 * @file tcp.c
 * @brief Polling TCP client for SecureOS.
 *
 * Purpose:
 *   Implements a minimal blocking TCP client: three-way handshake
 *   (SYN → SYN-ACK → ACK), data send (PSH+ACK), data receive until
 *   FIN or timeout, and graceful close (FIN+ACK).  All I/O is polling-based;
 *   no interrupts are used.  Intended for single-connection HTTP transactions.
 *
 * Interactions:
 *   - ipv4.c: TCP segments are sent via ipv4_send(); incoming segments arrive
 *     via tcp_dispatch() called from ipv4_poll().
 *   - http.c: calls tcp_connect / tcp_send / tcp_recv / tcp_close.
 *   - ipv4.c: ipv4_set_active_tcp_conn() registers the active socket so that
 *     ipv4_poll() can route incoming segments here.
 *
 * Limitations (v1):
 *   - Single connection at a time (no multiplexing).
 *   - No retransmit logic.
 *   - Window size is fixed at 65535.
 *   - Polling only (no SysTick or hardware timer).
 *
 * Launched by:
...
/* tcp.c declares this to register the conn with ipv4_poll dispatch */
...
/* Simple ephemeral port counter */
static uint16_t g_ephemeral_port = TCP_EPHEMERAL_BASE;

/* Receive ring – populated by tcp_dispatch */
...
static int tcp_send_segment(tcp_conn_t *conn, uint8_t flags,
  ...
  static uint8_t seg[TCP_HEADER_LEN + NETWORK_MTU];
  uint16_t csum = 0u;
  size_t i = 0u;

  if (data_len > (size_t)NETWORK_MTU) {
    data_len = (size_t)NETWORK_MTU;
  }
  ...
  seg[12] = (uint8_t)((TCP_HEADER_LEN / 4u) << 4u); /* data offset */
  seg[13] = flags;
  tcp_put_be16(seg, 14u, (uint16_t)TCP_WINDOW);
  seg[16] = 0u; seg[17] = 0u; /* checksum placeholder */
  seg[18] = 0u; seg[19] = 0u; /* urgent pointer */

  for (i = 0u; i < data_len; ++i) {
    seg[TCP_HEADER_LEN + i] = data[i];
  }

  csum = tcp_checksum(NET_GUEST_IP, conn->remote_ip,
                       seg, TCP_HEADER_LEN + data_len);
  tcp_put_be16(seg, 16u, csum);

  return ipv4_send(conn->remote_ip, IPV4_PROTO_TCP,
                   seg, TCP_HEADER_LEN + data_len);
}

/* -----------------------------------------------------------------------
 * TCP dispatch (called from ipv4_poll)
 * --------------------------------------------------------------------- */
...
void tcp_dispatch(uint32_t src_ip, const uint8_t *tcp_packet, size_t tcp_len,
  ...
  uint32_t seq = 0u, ack_num = 0u;
  uint8_t  data_offset = 0u, flags = 0u;
  size_t   header_len = 0u, payload_len = 0u;
...
tcp_result_t tcp_connect(tcp_conn_t *conn, uint32_t remote_ip, uint16_t remote_port) {
  ...
  conn->seq          = 0xA1B2C3D4u; /* fixed ISN (v1; no randomisation) */
  conn->ack          = 0u;
  conn->connected    = 0;
  conn->fin_received = 0;
  ...
  conn->seq += 1u; /* SYN consumes one sequence number */
...
tcp_result_t tcp_send(tcp_conn_t *conn, const uint8_t *data, size_t len) {
  ...
  while (sent < len) {
    ...
    if (chunk > (size_t)NETWORK_MTU) {
      chunk = (size_t)NETWORK_MTU;
    }
