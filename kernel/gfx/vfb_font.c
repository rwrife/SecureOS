/**
 * @file vfb_font.c
 * @brief Kernel-side 5x7 bitmap font for rendering text into session VFBs.
 *
 * Purpose:
 *   When a WM-managed session produces console text output, the kernel uses
 *   this font to render characters as pixels into the session's virtual
 *   framebuffer (VFB). This allows the window manager to always read pixels
 *   from the VFB regardless of whether the session is running text or
 *   graphics applications.
 *
 * Interactions:
 *   - session_manager.c: calls vfb_font_draw_char to render each character
 *   - console.c: text output triggers VFB rendering for WM-managed sessions
 *
 * Launched by:
...
static const unsigned char g_font_data[95][7] = {
...
void vfb_font_draw_char(unsigned char *buffer, int stride,
  ...
  glyph = g_font_data[idx - 0x20];

  for (row = 0; row < VFB_FONT_H; row++) {
    unsigned char bits = glyph[row];
    int dy = py + row;
    if (dy < 0 || dy >= 200) continue;
    for (col = 0; col < VFB_FONT_W; col++) {
      if (bits & (0x10 >> col)) {
        int dx = px + col;
        if (dx >= 0 && dx < stride) {
          buffer[dy * stride + dx] = color;
        }
      }
    }
  }
...
void vfb_font_draw_string(unsigned char *buffer, int stride,
  ...
  while (*str != '\0') {
    vfb_font_draw_char(buffer, stride, x, py, *str, color);
    x += VFB_FONT_W + VFB_FONT_SPACING;
    str++;
  }
}
